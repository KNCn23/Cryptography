...
uint64_t fromHexStringToLong (char* block){
    uint64_t result = 0; // Başlatma eklendi
    for (int i = 0; i < 16; i++)
...
char* ncrypt(char* plaintext, char* key){
...
// Test main function
int  main(){
    char *plaintext = malloc(17 * sizeof(char)); // 16 karakter + null terminator için yer ayır
    char *key = malloc(21 * sizeof(char)); // 20 karakter + null terminator için yer ayır
    char *ciphertext;

    printf("Enter the plaintext (64 bits) in hexadecimal format\nUse lower case characters and enter new line at the end\n");
    fgets(plaintext, 17, stdin); // gets yerine fgets kullan
    plaintext[strcspn(plaintext, "\n")] = 0; // Satır sonu karakterini kaldır

    //printf("Enter the key (80 bits) in hexadecimal format\nUse lower case characters and enter new line at the end\n");
    //fgets(key, 21, stdin); // gets yerine fgets kullan
    //key[strcspn(key, "\n")] = 0; // Satır sonu karakterini kaldır

    ciphertext = ncrypt(plaintext, key);
    printf("The ciphertext is: %s\n", ciphertext); // puts yerine printf kullan

    char *decryptedtext = decrypt(ciphertext, key);
    printf("The decrypted plaintext is: %s\n", decryptedtext); // puts yerine printf kullan

    free(key);
    free(plaintext);
    free(ciphertext);
    free(decryptedtext); // decrypt fonksiyonundan dönen belleği de serbest bırak
